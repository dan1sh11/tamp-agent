...
def build_plan(instruction: Instruction):
    ...
    return problem, plan_text, actions
...
def plan():
    ...
    try:
        problem, plan_text, actions = build_plan(instruction)
        return jsonify({
            "instruction": instruction.model_dump(),
            "problem": problem,
            "plan": plan_text,
            "actions": [{"type": a.type.value, "parameters": a.parameters} for a in actions],
            "execution": "Plan validated. You can execute it locally through the web UI."
        })
...
def execute():
    ...
    try:
        ...
        PlanExecutor(env).execute(actions)
        return jsonify({"success": True, "instruction": instruction.model_dump(), "problem": problem, "plan": plan_text})
    except Exception as exc:
        return jsonify({"success": False, "error": str(exc), "instruction": instruction.model_dump()}), 500
    finally:
        if env is not None:
            env.close()
