    ...
    def generate_problem(
        ...
        if instruction.action == "unknown":
            raise PDDLGenerationError(
                instruction.error or "Unknown instruction."
            )

        if instruction.object not in SCENE_OBJECTS:
            raise PDDLGenerationError(
                f"Unknown object: {instruction.object}"
            )

        if (
            instruction.action == "place"
            and instruction.target not in RECEPTACLES
        ):
            raise PDDLGenerationError(
                f"Unknown target: {instruction.target}"
            )
...
"""
    ...
    def _generate_goal(self, instruction: Instruction) -> str:
        ...
        if instruction.action == "move":
            raise PDDLGenerationError(
                "Move is not yet represented in the symbolic domain."
            )

        raise PDDLGenerationError(
            f"Unsupported action: {instruction.action}"
        )